...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update the details of this feedback record.</p>
    </div>

    <x-ui.card>
        <form action="{{ route('analyst.reviews.update', $review) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <x-ui.form-group label="Target Dataset" name="dataset_id" required class="mb-6">
                <select name="dataset_id" id="dataset_id" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white">
                    ...
                    @foreach($datasets as $dataset)
                        <option value="{{ $dataset->id }}" {{ old('dataset_id', $review->dataset_id) == $dataset->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <x-ui.form-group label="Author Name (Optional)" name="author_name">
                    <x-ui.input type="text" name="author_name" id="author_name" value="{{ old('author_name', $review->author_name) }}" placeholder="e.g. John Doe" />
                </x-ui.form-group>

                <x-ui.form-group label="Rating (1-5)" name="rating">
                    <x-ui.input type="number" name="rating" id="rating" value="{{ old('rating', $review->rating) }}" min="1" max="5" placeholder="5" />
                </x-ui.form-group>
            </div>

            <x-ui.form-group label="Review Content" name="content" required class="mb-6">
                <textarea name="content" id="content" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white" placeholder="Enter the full review text here...">{{ old('content', $review->content) }}</textarea>
            </x-ui.form-group>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <x-ui.form-group label="Processing Status" name="processed_status" required>
                    <select name="processed_status" id="processed_status" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white">
                        <option value="pending" {{ old('processed_status', $review->processed_status) == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="processed" {{ old('processed_status', $review->processed_status) == 'processed' ? 'selected' : '' }}>Processed</option>
                        <option value="error" {{ old('processed_status', $review->processed_status) == 'error' ? 'selected' : '' }}>Error</option>
                    </select>
                </x-ui.form-group>

                <x-ui.form-group label="Published Date (Optional)" name="published_at">
                    <x-ui.input type="date" name="published_at" id="published_at" value="{{ old('published_at', optional($review->published_at)->format('Y-m-d')) }}" />
                </x-ui.form-group>
            </div>
