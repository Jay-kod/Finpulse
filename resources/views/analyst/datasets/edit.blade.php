...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Dataset</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update configuration for {{ $dataset->name }}.</p>
    </div>

    <x-ui.card>
        <form action="{{ route('analyst.datasets.update', $dataset) }}" method="POST">
            @csrf
            @method('PUT')

            <x-ui.form-group label="Dataset Name" name="name" required class="mb-6">
                <x-ui.input type="text" name="name" id="name" value="{{ old('name', $dataset->name) }}" required placeholder="e.g. OPay Q1 Reviews 2026" />
            </x-ui.form-group>
            ...
            <x-ui.form-group label="Target Application" name="fintech_app_id" required class="mb-6">
                <select name="fintech_app_id" id="fintech_app_id" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white">
                    ...
                    @foreach($apps as $app)
                        <option value="{{ $app->id }}" {{ old('fintech_app_id', $dataset->fintech_app_id) == $app->id ? 'selected' : '' }}>
                            {{ $app->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <x-ui.form-group label="Source" name="source" required>
                    <select name="source" id="source" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white">
                        <option value="Google Play" {{ old('source', $dataset->source) == 'Google Play' ? 'selected' : '' }}>Google Play Store</option>
                        <option value="App Store" {{ old('source', $dataset->source) == 'App Store' ? 'selected' : '' }}>Apple App Store</option>
                        <option value="Twitter" {{ old('source', $dataset->source) == 'Twitter' ? 'selected' : '' }}>Twitter / X</option>
                        <option value="Custom" {{ old('source', $dataset->source) == 'Custom' ? 'selected' : '' }}>Custom Import</option>
                    </select>
                </x-ui.form-group>

                <x-ui.form-group label="Record Count" name="record_count" required>
                    <x-ui.input type="number" name="record_count" id="record_count" value="{{ old('record_count', $dataset->record_count) }}" required min="0" />
                </x-ui.form-group>
            </div>
            ...
            <x-ui.form-group label="Processing Status" name="status" required class="mb-8">
                <select name="status" id="status" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:text-white">
                    ...
                    <option value="processing" {{ old('status', $dataset->status) == 'processing' ? 'selected' : '' }}>Processing</option>
                    <option value="completed" {{ old('status', $dataset->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                    <option value="failed" {{ old('status', $dataset->status) == 'failed' ? 'selected' : '' }}>Failed</option>
                </select>
            </x-ui.form-group>
            ...
            <div class="flex items-center justify-end border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <x-ui.button type="submit" variant="primary">Update Dataset</x-ui.button>
            </div>
        </form>
    </x-ui.card>
</div>
